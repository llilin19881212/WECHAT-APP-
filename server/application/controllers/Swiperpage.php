...
class Swiperpage extends CI_Controller {
...
$sql = " SELECT `title` FROM `swiper`limit $swnum";
...
$sql = " SELECT article FROM `swiper`limit $swnum";
mysqli_query($conn, "set id 'utf8'");
...
$sql = " SELECT swnum FROM `swiper`limit $swnum";
mysqli_query($conn, "set id 'utf8'");
$result = mysqli_query($conn, $sql);
...
$rs=array($title,'swiper'.$image,$article);
