...
class Upimg1 extends CI_Controller {
...
$sql = " SELECT `title` FROM `preview`limit $artnum1";
...
$sql = " SELECT `author` FROM `preview`limit $artnum1";
mysqli_query($conn, "set id 'utf8'");
...
$sql = " SELECT `article` FROM `preview`limit $artnum1";
mysqli_query($conn, "set id 'utf8'");
...
$sql = " SELECT MAX(artnum) FROM `news`";
mysqli_query($conn, "set id 'utf8'");
...
$sql = "INSERT INTO news (title,author,article,artnum,auid)
VALUES ( '$title', '$author','$article','$artnum','$auid')";
